# 拡張機能ガイド — MCP・スキル・エージェント・プラグイン

Claude Code は「標準搭載の機能」だけでなく、外部ツールや自分で作ったルールを組み込むことで大幅に強化できる。このドキュメントでは、拡張機能を **探して・追加して・自分で作る** 方法を説明する。
...
Model Context Protocol（MCP）は、Claude が外部ツールを呼び出すための標準仕様。サーバーを追加すると、Claude が直接 GitHub を操作したり、ブラウザを動かしたりできるようになる。
...
「〇〇をしたい」という目的を伝えると、適切なサーバーを提案してくれる:

```
> GitHubのIssueを Claude から操作したい。
...
特定の役割・専門知識を持つサブエージェントの定義。`description` に「いつ使うか」を書いておくと、Claude が文脈を読んで自動的に呼び出す。

`03-advanced.md` でサブエージェントの仕組みは説明済み。ここでは「どう作るか」にフォーカスする。
...
- [05-dotfiles.md](./05-dotfiles.md) — dotfilesでチーム設定を共有する方法
- [03-advanced.md](./03-advanced.md) — MCPとエージェントのより詳しい使い方
